...
def load_config(args):
    ...
    assert cfg.data.object.input_segments[0] < cfg.data.object.input_segments[1], 'data.object.input_segments must be in an increasing order'

    # only support most naive setting
    if cfg.data.image.fps > 0 and cfg.data.object.fps > 0:
        assert cfg.data.image.fps == cfg.data.object.fps
    ...
    if cfg.model.obj_feat_size > 0:
        if cfg.data.object.input_from_annotated_segments:
            assert str(cfg.data.object.input_segments) == str(cfg.data.input_segments)
